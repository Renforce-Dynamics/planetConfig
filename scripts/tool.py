...
def main():
    # The configuration CLI owns its public help.
    p = argparse.ArgumentParser(description=__doc__, add_help=not (len(sys.argv) > 1 and sys.argv[1] in {"run"}))
    ...
    if args.action == "setup":
        ...
        if args.action == "test":
            ...
            invoke([python] + CONFIG[args.action] + extra,
                   cwd=None if args.action in {"run"} else ROOT)
